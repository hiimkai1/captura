...
from captura.models import User, Photo, Tag, Comment, Notification

fake = Faker()


def fake_admin():
    admin = User(name=' Talon Smart',
                 username='greyli',
    ...
    notification = Notification(message='Hello, welcome to captura.', receiver=admin)
...
def fake_photo(count=30):
    # photos
    upload_path = current_app.config['captura_UPLOAD_PATH']
    for i in range(count):
        print(i)
